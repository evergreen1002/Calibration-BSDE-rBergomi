...
"""
# Test: whether soe can support BP or not： Yes!
...
beta = 0.5-H
reps = 1e-4
# soe = SHIDONGJ(beta, reps, tau, T)
# Lambda_true, Omega_true, Nexp = soe.main()

soe_tf = SHIDONGJ_tf(tf.cast(0.5 - H, dtype = tf.float64), reps, tau, T)
Lambda_tf, Omega_tf, Nexp_tf = soe_tf.main()


# print(Lambda, Omega, Nexp, "\n")
print(Lambda_tf, Omega_tf, Nexp_tf)

class model(tf.keras.Model):
    def __init__(self):
        super(model, self).__init__()

        self.H = tf.Variable(np.squeeze(np.random.uniform(low = 0.01, high = 0.2, size = 1)),trainable= True, dtype = tf.float64)
        self.reps = 1e-4
        self.tau = 1e-3
        self.T = 1
    ...
    def simulation(self):
        beta = 0.5 - self.H
        soe = SHIDONGJ_tf(beta, self.reps, self.tau, self.T)
        Lambda, Omega, _ = soe.main()

        return Lambda, Omega

    def loss_fn(self, true_lambda, true_omega):
        Lambda, Omega = self.simulation()
        a = tf.reduce_sum((true_lambda - Lambda)**2)
        b = tf.reduce_sum((true_omega - Omega)**2)
...
with tf.GradientTape(persistent=True) as tape:
    loss = test.loss_fn(Lambda_true, Omega_true)

grad = tape.gradient(loss, test.H)
print(grad)
"""
