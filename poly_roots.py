...
def roots_tf(p):
    ...
    if N > 1:
        ...
        """
        A = tf.TensorArray(tf.float64, size = 0, dynamic_size=True, clear_after_read = False)
        B = tf.linalg.diag(tf.ones((N - 2,), dtype=tf.float64), k=-1)
        tf.print(B)        
        A = A.write(0, -p[1:]/p[0])
...
"""
# Test
p = tf.constant(np.arange(1, 6), dtype = tf.float64)
print(roots_tf(p), "\n")
# print(np.roots(p.numpy()))
"""
