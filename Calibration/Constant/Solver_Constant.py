...
class FullModel(tf.keras.Model):
    ...
    def simulation(self, time_steps, num_samples):        
        tf.random.set_seed(self.seed) 
        params = {"X0": self.X0, "V0": self.V0 , 
                  "xi": self.m_param.xi, 
                  "H": self.m_param.H,
                  "rho": self.m_param.rho,
                  "nu": self.m_param.eta,                    
                  }
        beta = 0.5 - self.m_param.H
        reps = 1e-4        
        soe = SHIDONGJ_tf(beta, reps, self.T/time_steps, self.T)
        Lambda, Omega, _ = soe.main()
        rbergomi = rBergomi_SOE_tf(time_steps, self.T, params, num_samples, Lambda, Omega)
    ...
    def call(self):
        ...
        for j in range(1, self.N+1):
            ...
            y_theta.append(tf.math.exp(-int_r) * payoff)
        
        y_theta = tf.concat(y_theta, axis = 1)
        y_theta = tf.reshape(tf.math.reduce_mean(y_theta, axis = 0), [self.N, self.L])
        # cubic spline interpolation on T
        y_theta_interp = tf.TensorArray(tf.float64, size = 0, dynamic_size = True, clear_after_read = False)           
        for i in range(self.L):
            spline = tff.math.interpolation.cubic.build_spline(self.grid, y_theta[:, i])
            price_i = tff.math.interpolation.cubic.interpolate(self.T_hist, spline)
            y_theta_interp = y_theta_interp.write(i, price_i)
        
        # size (NxL, )
        y_theta_interp = tf.reshape(tf.transpose(y_theta_interp.stack()), [-1])
        ...
        for j in range(1, self.N+1):
            ...
            # size (valid_size, L)
            payoff = tf.reshape(tf.math.exp(x[:, indx] + self.integ_r(T_j)), [-1, 1]) - tf.reshape(self.K, [1, -1])
            payoff = tf.where(payoff < 0, x = 0, y = payoff)   
...
class BSDESolver(object):
    ...
    def rele_err(self, true_y, y):        
        err = np.abs(true_y - y)/true_y
        max_err = np.max(err)
        avg_err = np.mean(err)
    ...
    def loss_fn(self):        
        ...
        aim_max_err, aim_avg_err = self.rele_err(self.true_y, y_theta) 
