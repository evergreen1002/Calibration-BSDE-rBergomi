...
from Solver_Function import BSDESolver
import numpy as np
import tensorflow as tf


flags.DEFINE_string('config_path', 'rBergomi_func.json', """The path to load json file.""")
...
def main(argv):
    ...
    for i in range(config.net_config.num_iterations):                  
        ...
        with tf.GradientTape() as tape:            
            loss, aim_list, xi = bsde_solver.loss_fn()        
        loss_history.append([loss])        
        aim_history.append(aim_list)
        xi_history.append(xi.numpy())
        param_history.append([H.numpy(), rho.numpy(), eta.numpy()])
    ...
    np.save("loss_history_5_func_1.npy", np.array(loss_history))
    np.save("aim_history_5_func_1.npy", np.array(aim_history)) 
    np.save("xi_5_func_1.npy", np.array(xi_history))
    np.save("param_5_func_1.npy", np.array(param_history))
