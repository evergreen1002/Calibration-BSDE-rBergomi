...
class FCNN(tf.keras.Model):
    def __init__(self, config):        
        super().__init__()
        self.num_hiddens = config.xi_config.num_hiddens
        ...
        self.dense_layers.append(tf.keras.layers.Dense(1, 
                                                    ...
                                                    bias_initializer = tf._initializers.Constant(value = config.xi_config.init_xi),
                                                    activation = None))
...
class FullModel(tf.keras.Model):
    ...
    # compute the xi_0(t) on a given grid 
    def call_xi(self, steps):
        return tf.math.abs(tf.reshape(self.xi_NN.call(tf.linspace(0, self.T, steps + 1)[1:]), [1, -1]))   

    def simulation(self, time_steps, num_samples): 
        xi = self.call_xi(time_steps)          
    ...
    def call(self):
        ...
        _, _, _, x, xi = self.simulation(1000, 10000)
        y_aim = []
        h = 1000/self.N
...
class BSDESolver(object):
    ...
    def loss_fn(self):        
        loss, y_theta, xi = self.model.call() 
